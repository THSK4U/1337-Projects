...
def extract_functions_infos(fn: Any) -> FunctionInfo:
    ...
    return_type = type_hints.pop('return').__name__ if 'return' in type_hints else "Any"
    
    type_map = {"int": "number", "float": "number", "str": "string", "bool": "boolean"}
    parameters = {k: {"type": type_map.get(v.__name__, v.__name__)} for k,v in type_hints.items()}
    returns = {"type": type_map.get(return_type, return_type)}
