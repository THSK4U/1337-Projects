...
def gameEventGenerator(n_events):
    for i in range(len(n_events)):
        player = n_events[i]['player']
        action = n_events[i]['event_type']
        if action == "kill":
...
def main():
    ...
    my_stream = gameEventGenerator(data_list)

    processStream(my_stream)
